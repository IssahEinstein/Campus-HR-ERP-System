...
export default function ProfileSettings() {
  ...
  const submitProfile = async (event) => {
    ...
    try {
      ...
      setError(requestError.response?.data?.detail ?? "Failed to update profile.");
    } finally {
  ...
  const submitPassword = async (event) => {
    ...
    try {
      ...
      setError(requestError.response?.data?.detail ?? "Failed to update password.");
    } finally {
  ...
  const uploadAvatar = async (event) => {
    ...
    try {
      ...
      setError(requestError.response?.data?.detail ?? "Failed to upload profile photo.");
    } finally {
  ...
  const removeAvatar = async () => {
    ...
    try {
      ...
      setError(requestError.response?.data?.detail ?? "Failed to remove profile photo.");
    } finally {
      setSavingAvatar(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div>
        <h1 className="text-3xl font-light mb-2">
          Profile <span className="font-medium" style={{ color: "#00523E" }}>Settings</span>
        </h1>
        <p className="text-gray-600">Update your name, password, profile picture, and description.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 flex items-center gap-4">
        {avatarUrl ? (
          ...
          <div className="w-16 h-16 rounded-full flex items-center justify-center text-white text-xl font-semibold" style={{ backgroundColor: "#00523E" }}>
            {initials}
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-500">Accepted: JPG, PNG, WEBP, GIF (max 5MB)</p>
          <p className="text-xs text-gray-500">Uploaded images are center-cropped and resized for consistent avatars.</p>
        </div>
      </div>
